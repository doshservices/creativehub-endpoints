...
exports.fundWalletWithSavedCard = async (req, res) => {
  const userId = req.user._id;
  req.body["userId"] = userId;
  try {
    const transaction = await new Wallet(req.body).fundWalletWithSavedCard();
    return success(res, { transaction });
  } catch (err) {
    logger.error(`Error crediting user ${userId} wallet. Error:`, err);
    return error(res, { code: err.code, message: err.message });
...
exports.verifyWithdrawFund = async (req, res) => {
  const userId = req.user._id;
  req.body["userId"] = userId;
  try {
    const withdrawal = await new Wallet(req.body).verifyWithdrawFund();
    return success(res, { withdrawal });
  } catch (err) {
    logger.error(
      `Error withdrawing user [${userId}] funds to bank account. Error:`,
